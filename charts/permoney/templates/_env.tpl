...
Usage examples (indent with nindent in caller):
  ...
  {{ include "permoney.env" (dict "ctx" . "includeDatabase" true "includeRedis" true "extraEnv" .Values.worker.extraEnv "extraEnvFrom" .Values.worker.extraEnvFrom) | nindent 10 }}
...
- name: SECRET_KEY_BASE
  valueFrom:
    secretKeyRef:
      name: {{ include "permoney.appSecretName" $ctx }}
      key: SECRET_KEY_BASE
...
{{- end }}
{{- if $includeDatabase }}
{{- $dburl := include "permoney.databaseUrl" $ctx -}}
{{- if $dburl }}
- name: DB_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ include "permoney.dbSecretName" $ctx }}
      key: {{ include "permoney.dbPasswordKey" $ctx }}
- name: DATABASE_URL
  value: {{ $dburl | quote }}
{{- end }}
{{- end }}
{{- if $includeRedis }}
{{- $redis := include "permoney.redisUrl" $ctx -}}
{{- if $redis }}
- name: REDIS_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ include "permoney.redisSecretName" $ctx }}
      key: {{ include "permoney.redisPasswordKey" $ctx }}
- name: REDIS_URL
  value: {{ $redis | quote }}
{{- $sentinelHosts := include "permoney.redisSentinelHosts" $ctx -}}
{{- if $sentinelHosts }}
- name: REDIS_SENTINEL_HOSTS
  value: {{ $sentinelHosts | quote }}
- name: REDIS_SENTINEL_MASTER
  value: {{ include "permoney.redisSentinelMaster" $ctx | quote }}
{{- end }}
{{- end }}
{{- end }}
