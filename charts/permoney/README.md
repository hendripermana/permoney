...
```sh
# Namespace
kubectl create ns permoney || true

# Install chart (example: provide SECRET_KEY_BASE and pin an immutable image tag)
helm upgrade --install permoney charts/permoney \
  -n permoney \
...
Expose the app via an Ingress (see values) or `kubectl port-forward svc/permoney 8080:80 -n permoney`.
...
rails:
  extraEnv:
    DATABASE_URL: postgresql://user:pass@db.example.com:5432/permoney
...
rails:
  existingSecret: permoney-secrets
  encryptionEnv:
    enabled: true
  settings:
    SELF_HOSTED: "true"
...
- The Redis password is taken from `permoney.redisSecretName` (typically your app Secret, e.g. `permoney-secrets`) using the key returned by `permoney.redisPasswordKey` (default `redis-password`).
...
Quickstart example (Sentinel, 3 replicas, Longhorn storage, reuse `permoney-secrets` password):
  ...
  managed:
    ...
    topologySpreadConstraints:
      - maxSkew: 1
        ...
        labelSelector:
          matchLabels:
            app.kubernetes.io/instance: permoney  # verify labels on your cluster
```
...
helm template permoney charts/permoney -n permoney -f ha-values.yaml --debug > rendered.yaml

# Install/upgrade in a test namespace
kubectl create ns permoney-test || true
helm upgrade --install permoney charts/permoney -n permoney-test -f ha-values.yaml --wait

# Verify CRs include your scheduling config
kubectl get cluster.postgresql.cnpg.io permoney-db -n permoney-test -o yaml \
  | yq '.spec | {instances, minSyncReplicas, maxSyncReplicas, nodeSelector, affinity, tolerations, topologySpreadConstraints}'

# Default RedisReplication CR name is <fullname>-redis (e.g., permoney-redis) unless overridden by redisOperator.name
kubectl get redisreplication permoney-redis -n permoney-test -o yaml \
...
kubectl delete pod -n permoney-test -l cnpg.io/cluster=permoney-db --wait=false --field-selector=status.phase=Running

# RedisReplication: delete one replica pod to let the operator recreate it under new constraints
kubectl delete pod -n permoney-test -l app.kubernetes.io/component=redis --wait=false

# Confirm distribution across nodes
kubectl get pods -n permoney-test -o wide
```

## Example app Secret (permoney-secrets)
...
cnpg:
  cluster:
    existingSecret: permoney-secrets   # if you are reusing the same Secret for DB creds
    secret:
...
2. `permoney-migrate` Job (post-install/post-upgrade hook) waits for the RW service to accept connections.
...
```sh
kubectl exec -n permoney deploy/$(kubectl get deploy -n permoney -o name | grep worker | cut -d/ -f2) -- \
...
```sh
helm template permoney charts/permoney -n permoney --debug > rendered.yaml  # dry-run inspection
helm upgrade --install permoney charts/permoney -n permoney --create-namespace --wait
kubectl get pods -n permoney
```

## Uninstall

```sh
helm uninstall permoney -n permoney
```

## Cleanup & reset (k3s)

For local k3s experimentation it’s sometimes useful to completely reset the `permoney` namespace, especially if CR finalizers or PVCs get stuck.
...
NAMESPACE=${NAMESPACE:-permoney}
RELEASE=${RELEASE:-permoney}

echo "[permoney-cleanup] Cleaning up Helm release '$RELEASE' in namespace '$NAMESPACE'..."

helm uninstall "$RELEASE" -n "$NAMESPACE" || echo "[permoney-cleanup] Helm release not found or already removed."
...
if kubectl get redisreplication.redis.redis.opstreelabs.in -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Clearing finalizers from RedisReplication CRs..."
...
if kubectl get redissentinels.redis.redis.opstreelabs.in -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Clearing finalizers from RedisSentinel CRs..."
...
if kubectl get pvc -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Clearing finalizers from PVCs..."
...
if kubectl get redisreplication.redis.redis.opstreelabs.in -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Deleting RedisReplication CRs (no wait)..."
  kubectl delete redisreplication.redis.redis.opstreelabs.in -n "$NAMESPACE" --all --wait=false || true
fi

if kubectl get redissentinels.redis.redis.opstreelabs.in -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Deleting RedisSentinel CRs (no wait)..."
...
if kubectl get cluster.postgresql.cnpg.io -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Deleting CNPG Cluster CRs (no wait)..."
  kubectl delete cluster.postgresql.cnpg.io -n "$NAMESPACE" --all --wait=false || true
fi

if kubectl get pvc -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Deleting PVCs in namespace $NAMESPACE (no wait)..."
  kubectl delete pvc -n "$NAMESPACE" --all --wait=false || true
fi

# 3) Delete namespace
if kubectl get ns "$NAMESPACE" >/dev/null 2>&1; then
  echo "[permoney-cleanup] Deleting namespace $NAMESPACE..."
  kubectl delete ns "$NAMESPACE" --wait=false || true
else
  echo "[permoney-cleanup] Namespace $NAMESPACE already gone."
fi

echo "[permoney-cleanup] Done."
```
