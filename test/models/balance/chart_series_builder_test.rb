...
class Balance::ChartSeriesBuilderTest < ActiveSupport::TestCase
  ...
  test "linked account with orphaned currency balances shows correct values after cleanup" do
    ...
    # Create correct balances in EUR
    create_balance(account: linked_account, date: 1.day.ago.to_date, balance: 900)
    create_balance(account: linked_account, date: Date.current, balance: 1200)
    ...
    series = builder.balance_series
    # Should only include EUR balances, ignoring the orphaned USD balance
    # EUR balances are used: 900 from 1 day ago and 1200 from today
    assert_equal [ 900, 1200 ], series.map { |v| v.value.amount }
  end
end
