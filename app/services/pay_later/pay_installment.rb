module PayLater
  class PayInstallment
    ...
    def call
      ...
      installment = PayLaterInstallment.for_account(account.id).find_by!(installment_no: params.fetch(:installment_no))
      ...
      ActiveRecord::Base.transaction do
        ...
        # Late fee calculation (two-step)
        late_days = (payment_date - installment.due_date).to_i
        late_days = [ late_days, 0 ].max
        fee = 0.to_d
        if late_days > 0
          first = [ late_days, 7 ].min
          rest = [ late_days - 7, 0 ].max
        ...
        # Pay via double-entry transfer: source(cash) -> destination(PayLater)
        ...
        # Ensure balances recompute for Net Worth
