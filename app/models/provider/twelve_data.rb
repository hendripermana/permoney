class Provider::TwelveData < Provider
  ...
  def fetch_exchange_rates(from:, to:, start_date:, end_date:)
    with_provider_response do
      # Try to fetch the currency pair via the time_series API (consumes 1 credit)
      ...
      # If currency pair is not available, try via time_series/cross (consumes 5 credits)
      if data.nil?
