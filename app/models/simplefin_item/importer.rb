...
class SimplefinItem::Importer
  ...
  def initialize(simplefin_item, simplefin_provider:, sync: nil)
    ...
    @reconciled_account_ids = Set.new
  end
  ...
  private
    ...
    def import_account_minimal_and_balance(account_data)
      ...
      if (acct = sfa.current_account)
        ...
        balance_value = is_liability ? normalized : observed
        cash_balance_value = if acct.accountable_type == "Investment"
          avail.nonzero? ? avail : balance_value
        end
    ...
    def fetch_accounts_data(start_date:, end_date: nil, pending: nil)
      ...
      setting_pending = Setting.respond_to?(:syncs_include_pending) ? Setting.syncs_include_pending : nil
      effective_pending = if pending.nil?
        setting_pending.nil? ? Rails.configuration.x.simplefin.include_pending : setting_pending
      else
        pending
      end
