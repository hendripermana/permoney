...
export default class extends Controller {
  ...
  setupEventListeners() {
    ...
    if (scene) {
      ...
      if (this.enableDragValue) {
        ...
        scene.addEventListener('touchstart', this.boundHandleDragStart, { passive: false })
      }
    }
  }
  ...
  prev() {
    ...
    this.isRotating = true
    const newIndex = this.currentItemIndex === 0 ? this.itemsValue.length - 1 : this.currentItemIndex - 1
    this.pendingIndexChange = newIndex
  ...
  handleAnimationComplete(triggeredBy) {
    if (this.isRotating && this.pendingIndexChange !== null) {
      ...
      if (currentBackFaceIndex === 0) {
        this.prevIndex = (this.pendingIndexChange + indexOffset + this.itemsValue.length) % this.itemsValue.length
      } else if (currentBackFaceIndex === 1) {
        this.currentIndex = (this.pendingIndexChange + indexOffset + this.itemsValue.length) % this.itemsValue.length
      } else if (currentBackFaceIndex === 2) {
        this.nextIndex = (this.pendingIndexChange + indexOffset + this.itemsValue.length) % this.itemsValue.length
      } else if (currentBackFaceIndex === 3) {
        this.afterNextIndex = (this.pendingIndexChange + indexOffset + this.itemsValue.length) % this.itemsValue.length
      }
  ...
  updateBackFace(faceIndex) {
    ...
    if (faces[faceIndex]) {
      const itemIndex = [this.prevIndex, this.currentIndex, this.nextIndex, this.afterNextIndex][faceIndex]
      const item = this.itemsValue[itemIndex]
