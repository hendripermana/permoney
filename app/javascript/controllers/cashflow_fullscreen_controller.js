...
export default class extends Controller {
  ...
  async handlePeriodChange(newPeriod) {
    ...
    try {
      ...
      if (newSankeyElement) {
        const newSankeyData = JSON.parse(newSankeyElement.getAttribute('data-cashflow-fullscreen-sankey-data-value'));
        const newCurrencySymbol = newSankeyElement.getAttribute('data-cashflow-fullscreen-currency-symbol-value');
        ...
        this.element.setAttribute('data-cashflow-fullscreen-sankey-data-value', JSON.stringify(newSankeyData));
        this.element.setAttribute('data-cashflow-fullscreen-currency-symbol-value', newCurrencySymbol);
  ...
  updateFooterStats() {
    const footerStats = this.fullscreenModal?.querySelector('.flex.items-center.justify-between.text-sm.text-secondary');
    if (footerStats && footerStats.children[0]) {
  ...
  updateMainPageChart(responseHtml) {
    ...
    const newSankeySection = doc.querySelector('[data-controller*="sankey-chart"]:not([id="fullscreen-sankey-container"])');
    const currentSankeySection = document.querySelector('[data-controller*="sankey-chart"]:not([id="fullscreen-sankey-container"])');
    ...
    if (newSankeySection && currentSankeySection) {
      ...
      const newCurrencyValue = newSankeySection.getAttribute('data-sankey-chart-currency-symbol-value');
      
      currentSankeySection.setAttribute('data-sankey-chart-data-value', newDataValue);
      currentSankeySection.setAttribute('data-sankey-chart-currency-symbol-value', newCurrencyValue);
      
      // Trigger chart redraw
      if (window.Stimulus) {
        const controller = window.Stimulus.controllers.find(c => 
          c.element === currentSankeySection && c.identifier === 'sankey-chart'
        );
  ...
  async handleExport() {
    try {
      const chartContainer = this.fullscreenModal?.querySelector('#fullscreen-sankey-container svg');
      if (!chartContainer) {
      ...
      backgroundRect.setAttribute('fill', getComputedStyle(document.documentElement).getPropertyValue('--color-background') || '#ffffff');
  ...
  detectDarkTheme() {
    ...
    if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
      ...
      const hasLightClass = htmlElement.classList.contains('light') || bodyElement.classList.contains('light');
      if (!hasLightClass) {
        return true;
      }
    }
