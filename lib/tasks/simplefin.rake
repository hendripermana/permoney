...
namespace :permoney do
  namespace :simplefin do
    ...
    task :debug, [ :item_id ] => :environment do |_, args|
      unless args[:item_id].present?
        puts({ error: "usage", example: "bin/rails permoney:simplefin:debug[ITEM_ID]" }.to_json)
        exit 1
      end
    ...
    task :encrypt_access_urls, [ :batch_size, :limit, :dry_run ] => :environment do |_, args|
      Rake::Task["permoney:encrypt_access_urls"].invoke(args[:batch_size], args[:limit], args[:dry_run])
    end
  end
  ...
  task :encrypt_access_urls, [ :batch_size, :limit, :dry_run ] => :environment do |_, args|
    ...
    raw_batch = args[:batch_size].presence || ENV["BATCH_SIZE"].presence || ENV["PERMONEY_BATCH_SIZE"].presence || ENV["SURE_BATCH_SIZE"].presence
    raw_limit = args[:limit].presence || ENV["LIMIT"].presence || ENV["PERMONEY_LIMIT"].presence || ENV["SURE_LIMIT"].presence
    raw_dry = args[:dry_run].presence || ENV["DRY_RUN"].presence || ENV["PERMONEY_DRY_RUN"].presence || ENV["SURE_DRY_RUN"].presence
...
namespace :sure do
  namespace :simplefin do
    desc "Print debug info for a SimpleFin item: latest sync, snapshot accounts, simplefin_accounts, and unlinked list"
    task :debug, [ :item_id ] => :environment do |_, args|
      Rake::Task["permoney:simplefin:debug"].invoke(args[:item_id])
    end

    desc "Encrypt existing SimpleFin access_url values (idempotent). Args: batch_size, limit, dry_run"
    task :encrypt_access_urls, [ :batch_size, :limit, :dry_run ] => :environment do |_, args|
      Rake::Task["permoney:simplefin:encrypt_access_urls"].invoke(args[:batch_size], args[:limit], args[:dry_run])
    end
  end
